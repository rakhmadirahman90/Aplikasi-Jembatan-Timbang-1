...
$(function() {
	...
	// get the available COM ports and add them to the combo list
	if (typeof chrome !== 'undefined' && chrome.serial && chrome.serial.getDevices) {
		...
		// Browser fallback: Add virtual COM ports
		var mockPorts = ['COM1 (Indicator Digital Scale)', 'COM2 (Secondary Display)', 'COM3 (RFID Reader)'];
		for (var i = 0; i < mockPorts.length; i++) {
			$("#serial_ports_combobox").append('<option value="' + mockPorts[i] + '">' + mockPorts[i] + '</option>');
		}
	}
	...
	$("#connect_button").bind("click", function(event, ui) {
		if (typeof chrome !== 'undefined' && chrome.serial && chrome.serial.connect) {
			if(!connected) {
			...
			updateGUI();
			if (connected) {
				console.log('[Scale Simulator] Connected to ' + $("#serial_ports_combobox").val());
				var defaultWeight = '12500';
				if (!$('#hasilkg').val()) $('#hasilkg').val(defaultWeight);
				if (!$('#hasilkgLama').val()) $('#hasilkgLama').val('28400');
				if (!$('#hasilkgkpcc').val()) $('#hasilkgkpcc').val(defaultWeight);
				if (!$('#hasilkgLamakpcc').val()) $('#hasilkgLamakpcc').val('28400');
			}
		}
	});
	...
	$("#send_button").bind("click", function(event, ui) {
		if(connected) {
			...
			if (typeof chrome !== 'undefined' && chrome.serial && chrome.serial.send) {
				chrome.serial.send(connectionId, convertStringToArrayBuffer(textToSend), function(sendInfo) {
					if(sendInfo.error) alert('Unable to send data: ' + sendInfo.error);
				});
			} else {
				console.log('[Scale Simulator] Sent data:', textToSend);
			}
		}
	});	
});
